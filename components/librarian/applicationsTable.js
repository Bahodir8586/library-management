...
const ApplicationsTable = () => {
  ...
  useEffect(() => {
    search();
  }, [searchBy.value, pageNumber, search]);

  const search = useCallback(() => {
    console.log(searchBy.value, searchText);
    axios
      .get(
        `/librarian/applications?searchBy=${searchBy.value}&searchText=${searchText}&page=${pageNumber}`
      )
      .then((response) => {
  ...
  }, [pageNumber, searchBy.value, searchText]);

  const acceptOrderHandler = (id) => {
    setShowAcceptModal(false);
    axios
      .post(`/librarian/order/accept/${id}`)
      .then((response) => {
        console.log(response);
  ...
  const rejectOrderHandler = useCallback(
    (id) => {
      ...
      axios
        .post(`/librarian/order/reject/${id}`, { message })
        .then((response) => {
          console.log(response);
  ...
  const rejectModal = useMemo(
    ...
    [message, rejectOrderHandler, selectedOrder.id, showRejectModal]
  );
  const acceptModal = useMemo(
    ...
    [selectedOrder.id, showAcceptModal]
  );
