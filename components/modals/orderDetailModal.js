      ...
      <div className={"border-b pb-3"}>
        <div className={"px-4"}>
          ...
          <div className={"flex text-left mb-2"}>
            ...
            <div
              className={`w-full font-medium ${!givenDate && "text-red-600 "}`}
            >
              {givenDate
                ? givenDate
                : status?.message === "denied"
                ? "This order is not accepted "
                : "Book is not given yet"}
            </div>
          </div>
          <div className={"flex text-left mb-2"}>
            ...
            <div
              className={`w-full font-medium ${
                !mustReturnDate && "text-red-600 "
              }`}
            >
              {order.mustReturnDate
                ? mustReturnDate
                : status?.message === "denied"
                ? "This order is not accepted "
                : "Book is not given yet"}
        ...
        {status?.message !== "finished" &&
          ...
          !notUser && (
            <div className={"mt-4"}>
              <button
                className={
                  "px-6 py-3 bg-green-600 text-white rounded text-xl cursor-pointer transition duration-200 hover:bg-green-700"
                }
