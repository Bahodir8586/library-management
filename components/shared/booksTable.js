...
const BooksTable = ({ books }) => {
  ...
  useEffect(() => {
    search();
  }, [filter.value, searchBy.value, sort.value, pageNumber, search]);
  ...
  const warningModal = useMemo(
    ...
    [selectedBook.id, selectedBook.name, showDeleteModal]
  );
      ...
      <div className="py-8">
        <div className="flex flex-row mb-1 sm:mb-0 justify-between w-full">
          <div className={"w-1/12"}>
            <h2 className="text-6xl leading-tight ml-2">Books</h2>
          </div>
          ...
          <div className="text-end w-11/12">
            ...
            >
              ...
              <div
                className={
                  "pt-4 relative w-1/6 px-4 flex items-center justify-around"
                }
              >
              ...
              <tbody>
                {data?.map((el) => (
                  <tr
                    key={el.id}
                    className={
                      "text-sm bg-white border-b border-gray-200 py-3 h-20"
                    }
                  >
                    <td className="px-5 text-center">
                      <Link href={`/${role}/books/${el.id}`} passHref={true}>
                      ...
                      <div className="text-gray-900 flex flex-wrap items-center justify-center">
                        {el.categories?.map((category) => (
                          <span
                            key={category.id}
                            className={
                              "px-4 py-1 mx-1 text-base rounded-full text-white  bg-indigo-500 "
                            }
                          >
                            {category.name}
                          </span>
