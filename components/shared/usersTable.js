...
const UsersTable = () => {
  ...
  const search = useCallback(() => {
    ...
    axios
      .get(
        `/users?filter=${filter.value}&searchText=${searchText}&searchBy=${searchBy.value}&page=${pageNumber}`
      )
      .then((response) => {
  ...
  }, [filter.value, pageNumber, searchBy.value, searchText]);

  const blockUserHandler = useCallback(
    (id) => {
      console.log(id);
      axios
        .post(`/librarian/users/block/${id}`, { message: message })
        .then((response) => {
          console.log(response);
  ...
  const unblockUserHandler = (id) => {
    console.log(id);
    axios
      .post(`/librarian/users/unblock/${id}`, {})
      .then((response) => {
        console.log(response);
  ...
  const warningModal = useMemo(
    ...
    [deleteUserHandler, selectedUser.id, showDeleteModal]
  );
  ...
  const messageModal = useMemo(
    ...
    [blockUserHandler, message, selectedUser.id, showMessageModal]
  );
    ...
    <div className="container mx-auto px-4 sm:px-8 w-full">
      ...
      <div className="py-8">
        <div className="flex flex-row mb-1 sm:mb-0 justify-between w-full items-center">
          <h2 className="text-6xl leading-tight w-1/4">Users</h2>
          <div className="text-end w-3/4">
            <form
              className="flex flex-col md:flex-row md:w-full md:space-x-3 space-y-3 md:space-y-0 justify-center w-full"
              onSubmit={(e) => {
                e.preventDefault();
                search();
              }}
